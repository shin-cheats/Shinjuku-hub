                                ...
                                if team ~= "Nextbot" and char.Name ~= LocalPlayer.Name and downed == true then
                                    ...
                                    if hrp and Camera then
                                        ...
                                        if onScreen then
                                            ...
                                            tracer.Color = Color3.fromRGB(255, 120, 120) -- soft red
                                            tracer.Thickness = 2
...
-- 7. Speed / JumpCap / Strafe sliders
...
MainTab:CreateToggle({
    ...
    Callback = function(state)
        ...
        -- original InfiniteSlide logic already uses RunService loops; kept intact
        local cachedTables
        local plrModel
...
MiscTab:CreateToggle({
    Name = "Lag Switch (Flag Removed)",
    CurrentValue = false,
    Flag = "RemovedFlagHAHAHHAHALOL",
    Callback = function(state)
...
MiscTab:CreateSlider({
    Name = "Lag Duration (Removed Flag)",
    Range = {0.1, 10},
    Increment = 0.1,
    Suffix = "s",
    CurrentValue = 0.5,
...
-- Lag switch trigger via hidden button (original behavior)
task.spawn(function()
    while true do
        if getgenv().lagSwitchEnabled then
            -- Do nothing; lag triggered from GUI button
        end
        task.wait()
    end
end)
